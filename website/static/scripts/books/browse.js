...
function getBookID(elt){
    while (elt && elt.tagName != "ARTICLE")
        elt = elt.parentNode

    return elt.id;
}

function viewBook(elt) {
    window.location.href = "/books/" + getBookID(elt)
}
        ...
        fetch("/api/books?page=" + page_number + "&search=" + document.getElementById("search").value + "&author=" + document.getElementById("author").value,
            {
                ...
                if (data.length > 0) {
                    ...
                    <button class="py-0.5 px-4 rounded-full bg-blue-700 hover:bg-blue-900" onclick="addtofav(this)">Add ❤️</button>
                    <button class="py-0.5 px-4 rounded-full bg-blue-700 hover:bg-blue-900" onclick="viewBook(this)">View 👀</button>
                    </h3>
